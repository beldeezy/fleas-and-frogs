...
export default function OnboardingPrioritizePage() {
  ...
  // ---- Confirm priorities (no routing here; shell handles navigation) ----
  ...
  // Guardrail: Next only enabled when canConfirm is true
  useEffect(() => {
    setCanProceed(canConfirm);
  }, [canConfirm, setCanProceed]);
    ...
    <section className="onboarding-step">
      ...
      {hydrated && (
        <>
              ...
              {priorities.map((priority, index) => (
                ...
                >
                  <div className="ff-task-main">
                    ...
                    <span className="ff-pill ff-pill-rank">{index + 1}.</span>
                    <span className="ff-priority-name">{priority.name}</span>
                  </div>
              ...
              {priorities.length === 0 && (
                <li className="ff-task ff-empty">
                  <span className="ff-hint">
                    No Life Priorities yet. Add at least one to start shaping
                    where you want your time to go.
                  </span>
                </li>
              )}
            </ul>
          ...
          {/* Confirm -> purely visual/psych, Next does nav */}
          <section className="ff-section">
            <button
              type="button"
              className="ff-button"
              onClick={handleConfirmPriorities}
              disabled={!canConfirm || prioritiesConfirmed}
            >
              {prioritiesConfirmed
            ...
            <p className="ff-hint">
              When every task has a Life Priority, the Next button will move you
              into the Strategize step.
            </p>
          </section>
        </>
      )}
